...
export async function middleware(request: NextRequest) {
    ...
    if (!decoded) return NextResponse.redirect(new URL(`${urlMain}/signin`, request.url));
    if (!!decoded && (session?.timeExpired || 0) > 0) {
        const pathnameLocal = pathname || '';
        const targetUrl = pathnameLocal.replace('/','').replaceAll('/', '010');
        return NextResponse.redirect(new URL(`${urlMain}/refresh?callbackUrl=${targetUrl}`, request.url));
    }
